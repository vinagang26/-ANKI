...
$mdPath = Join-Path -Path $PSScriptRoot -ChildPath "Liquid_Glass_Anki_Desktop_Guide.md"

$mdContent = @"
# 🚀 How Web Code Becomes a Desktop App: The Ultimate Beginner's Guide
...
## 1. 💡 The Big Picture: Web vs. Desktop Apps
...
## 2. 🛠️ How We Built Your Desktop App (3 Main Steps)
...
- Your flashcard UI is made of:
  ...
  - `scheduler.js` & `app.js` (Anki SM-2 SRS Algorithm)
...
- Instead of opening CMD and typing `python main.py` every time, double-clicking `run_app.bat` did it for you.
- **Why it couldn't be moved to Desktop:** A batch file uses *relative file paths*. If you move `run_app.bat` out of the folder to your Desktop, it can no longer find `main.py` or the `web/` folder!
...
## 🌐 3. Why the Google Translate API Still Works

In `app.js`, when you type Hanzi, the app makes an HTTP request to Google's translation API:
`https://translate.googleapis.com/translate_a/single?...`

- **In a web browser:** Browsers sometimes block cross-domain calls due to security rules called **CORS**.
- **In PyWebView Desktop App:** Native desktop webviews run under the local `file://` scheme, which bypasses CORS blocks! The app has full network access, so the Google API call works **100% reliably**.

---

## 🎓 4. Step-by-Step Learning Route (Roadmap for Beginners)
...
- **CSS**: Learn layout (`flexbox`, `grid`), colors, gradients, `backdrop-filter: blur()`, and glassmorphism styling.

### Stage 2: Interactive JavaScript & Algorithms
- **DOM Manipulation**: Learn how JavaScript changes text on screen when buttons are clicked (`addEventListener`, `innerHTML`).
- **APIs**: Learn how `fetch()` works to get data from web APIs (like Google Translate).
...
Write-Host "Created guide document at: $mdPath"
